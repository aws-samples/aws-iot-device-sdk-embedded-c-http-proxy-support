...
/**
 * @file subscribe_publish_proxy_sample.c
 * @brief simple MQTT publish and subscribe on the same topic via a designated HTTP proxy server.
 *
 * This example takes the parameters from the aws_iot_config.h file and establishes a connection to the AWS IoT MQTT Platform via an HTTP proxy.
...
int main(int argc, char **argv) {
	...
	mqttInitParams.proxyType = PROXY_HTTP;
	mqttInitParams.pProxyHostURL = HTTP_PROXY_HOST;
	mqttInitParams.proxyPort = HTTP_PROXY_PORT;
	mqttInitParams.isAuthenticationRequired = HTTP_PROXY_AUTH_ENABLE;
	mqttInitParams.pProxyUserName = HTTP_PROXY_USERNAME;
	mqttInitParams.pProxyPassword = HTTP_PROXY_PASSWORD;
	...
	while((NETWORK_ATTEMPTING_RECONNECT == rc || NETWORK_RECONNECTED == rc || SUCCESS == rc)
		...
		if(rc != SUCCESS) {
			IOT_WARN("QOS0 publish failed ret %x.\n", rc);
		}
